...
- **Database** (`/database`): Browse, search, edit, delete vocabulary entries. Select individual entries or use select-all to bulk delete. Import CSV, export to Excel. Filter by language, search text, or tags. Supports deep-linking with pre-applied filters via URL query params: `/database?tags=HS2.1,HS2.2` pre-selects specific tags, and `/database?prefix=HS2` pre-selects all tags matching the prefix (e.g. `HS2.1`, `HS2.2`). This enables external sites to link directly to a filtered view.
