...
**Prompt Templates**: Three Go string constants (`WordsTemplate`, `ExpressionsTemplate`, `SentenceTemplate`) parameterized by `{source_language}`, `{word}`/`{expression}`/`{sentence}`, `{context}`, and `{target_language_name}`. Words and expressions templates include a Core Rule Block and Decision Rubric. The sentence template focuses on grammar checking, correction, key vocabulary extraction, and translation. `BuildPrompt` resolves language codes to full names via the registry and replaces all placeholders using `strings.NewReplacer`.

**JSON Validation**: `ValidateResponse` parses raw LLM JSON against English schemas for three modes: `"words"`, `"expressions"`, and `"sentences"`. Required fields must be present; optional fields default to `""`. Translation fields (`english`, `target_translation`) are normalized from either plain strings or `{primary, alternatives}` objects into a consistent `Translation` struct. Sentence validation handles additional structured fields: `grammar_errors` (array of error/correction/explanation objects), `key_vocabulary` (array of word/definition/english objects), and `is_correct` (boolean). Returns `ValidationError` listing any missing or malformed fields.
